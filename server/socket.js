...
const setUpSockent = (server) => {
  ...
  const sendChannelMessage = async(message)=>{
    ...
    console.log("message creted ",createdMessage);
    ...
    if(channel && channel.members){
      channel.members.forEach((member)=>{
        const memberSockeId = userSocketMap.get(member._id.toString())

        if(memberSockeId){
          io.to(memberSockeId).emit("recieve-channel-message",finalData)
        }

      
      })
      const adminSockeId = userSocketMap.get(channel.admin._id.toString())

      if(adminSockeId){
        io.to(adminSockeId).emit("recieve-channel-message",finalData)
      }
    }
  }
